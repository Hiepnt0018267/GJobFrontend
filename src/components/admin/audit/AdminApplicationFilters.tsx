...
export type AdminApplicationStatusFilter = ApplicationStatus | 'ALL'

type Props = {
  search: string
  status: AdminApplicationStatusFilter
  onSearchChange: (value: string) => void
  onStatusChange: (value: AdminApplicationStatusFilter) => void
}

export default function AdminApplicationFilters({ search, status, onSearchChange, onStatusChange }: Props) {
    ...
    <section className="mt-6 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:p-5" aria-label="Tìm kiếm và lọc đơn ứng tuyển">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_240px]">
        <label className="relative block">
          <span className="sr-only">Tìm đơn ứng tuyển</span>
          <Search size={18} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true" />
          <input ref={inputRef} defaultValue={search} onChange={(event) => scheduleSearch(event.target.value)} placeholder="Tìm theo ứng viên, email, công việc hoặc công ty..." className="peer w-full rounded-xl border border-slate-300 py-3 pl-11 pr-11 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
        ...
        <label>
          ...
          <select value={status} onChange={(event) => onStatusChange(event.target.value as AdminApplicationStatusFilter)} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm font-medium text-slate-800 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100">
