...
export default function RecruiterSidebar({ collapsed = false, hoverExpanded = false, onHoverChange, onNavigate, onToggleCollapse, mobile = false }: Props) {
  ...
  return <aside className={mobile ? 'flex h-full w-72 flex-col bg-white' : 'sticky top-0 hidden h-screen min-w-0 flex-col border-r border-slate-200 bg-white lg:flex'} aria-label="Điều hướng nhà tuyển dụng" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
    <div className={`flex h-[68px] items-center border-b border-slate-200 ${expanded ? 'px-5' : 'justify-center px-0'}`}>
      <Link to="/recruiter" onClick={onNavigate} className={`flex min-w-0 items-center rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${expanded ? 'gap-3' : 'justify-center'}`} aria-label="GJob — Tổng quan tuyển dụng" title={compact && !hoverExpanded ? 'GJob — Tổng quan tuyển dụng' : undefined}>
        ...
        <span className={expanded ? 'min-w-0' : 'sr-only'}><span className="block text-base font-bold leading-5 text-slate-950">GJob</span><span className="mt-0.5 block text-xs font-medium text-slate-500">Khu vực nhà tuyển dụng</span></span>
      </Link>
    </div>
    <nav className={`flex-1 overflow-y-auto py-5 ${expanded ? 'px-3' : 'px-2'}`} aria-label="Các khu vực nhà tuyển dụng">
      <div className={expanded ? 'space-y-6' : 'space-y-5'}>{recruiterNavigationGroups.map((group) => <section key={group.label}><p className={expanded ? 'px-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400' : 'sr-only'}>{group.label}</p><div className={expanded ? 'mt-2 space-y-1' : 'space-y-2'}>{group.items.map(({ to, label, icon: Icon, isActive }) => {
