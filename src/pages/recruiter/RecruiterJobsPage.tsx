...
import { JOB_STATUSES, RECRUITER_JOB_SORT_OPTIONS } from '../../types/job'
import type { JobStatus, RecruiterJob, RecruiterJobListResponse, RecruiterJobSortOption } from '../../types/job'
...
const emptyData: RecruiterJobListResponse = { items: [], page: 1, page_size: 10, total: 0, total_pages: 0 }
...
export default function RecruiterJobsPage() {
  ...
  const sort = rawSort && RECRUITER_JOB_SORT_OPTIONS.includes(rawSort as RecruiterJobSortOption) ? rawSort as RecruiterJobSortOption : 'newest'
  ...
  const load = useCallback(async () => {
    ...
    try { const nextData = await recruiterJobService.getMyJobs({ keyword: keyword || undefined, status: jobStatus, sort, page, page_size: 10 }); if (currentRequest === requestId.current) setData(nextData) }
    catch (requestError) { if (currentRequest === requestId.current) setError(recruiterJobErrorMessage(requestError)) }
    finally { if (currentRequest === requestId.current) setLoading(false) }
  }, [keyword, jobStatus, sort, page])
  ...
  const act = async (job: RecruiterJob, action: 'delete' | 'close') => {
    ...
    setBusyId(job.id); setBusyAction(action); setError(null)
    try { if (action === 'delete') await recruiterJobService.deleteJob(job.id); else await recruiterJobService.closeJob(job.id); await load() }
    catch (requestError) { setError(recruiterJobErrorMessage(requestError)) }
  ...
  return <div className="min-h-screen bg-slate-50"><RecruiterHeader /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <section className="mt-7 flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 md:flex-row"><label className="relative min-w-0 flex-1"><span className="sr-only">Tìm tin tuyển dụng</span><Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={keyword} onChange={(event) => updateParam('keyword', event.target.value)} className="w-full rounded-xl border border-slate-300 py-2.5 pl-10 pr-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" placeholder="Tìm theo tiêu đề, công ty..." /></label><select aria-label="Lọc trạng thái" value={jobStatus ?? ''} onChange={(event) => updateParam('status', event.target.value)} className="rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-700"><option value="">Tất cả trạng thái</option><option value="PENDING">Chờ duyệt</option><option value="APPROVED">Đã duyệt</option><option value="REJECTED">Bị từ chối</option><option value="CLOSED">Đã đóng</option></select><select aria-label="Sắp xếp" value={sort} onChange={(event) => updateParam('sort', event.target.value)} className="rounded-xl border border-slate-300 px-3 py-2.5 text-sm text-slate-700"><option value="newest">Mới nhất</option><option value="oldest">Cũ nhất</option><option value="updated">Vừa cập nhật</option></select></section>
    {error && <div role="alert" className="mt-5 flex items-start justify-between gap-4 rounded-xl bg-red-50 p-4 text-sm text-red-700 ring-1 ring-red-100"><div className="flex gap-3"><AlertCircle size={18} className="mt-0.5 shrink-0" /><div><p className="font-semibold">Không thể hoàn tất yêu cầu</p><p>{error}</p></div></div><button type="button" onClick={() => void load()} className="shrink-0 rounded-lg border border-red-200 px-3 py-1.5 font-semibold hover:bg-red-100">Thử lại</button></div>}
