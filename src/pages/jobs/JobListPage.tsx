...
import { EMPLOYMENT_TYPES, EXPERIENCE_LEVELS, JOB_SORT_OPTIONS, type JobListResponse, type JobSearchParams } from '../../types/job'
...
function Skeleton() {
  return <div className="grid gap-4" aria-busy="true" aria-label="Đang tải danh sách việc làm"><div className="h-44 animate-pulse rounded-2xl bg-slate-200" /><div className="h-44 animate-pulse rounded-2xl bg-slate-200" /><div className="h-44 animate-pulse rounded-2xl bg-slate-200" /></div>
}
...
export default function JobListPage() {
  ...
  const filters = useMemo<JobFilterValues>(() => {
    ...
    const experienceLevel = parsedParams.get('experience_level')
    const sort = parsedParams.get('sort')
    return {
      employmentType: hasValue(EMPLOYMENT_TYPES, employmentType) ? employmentType : '',
      experienceLevel: hasValue(EXPERIENCE_LEVELS, experienceLevel) ? experienceLevel : '',
      salary: parsedParams.get('salary') ?? '',
  ...
  const requestParams = useMemo<JobSearchParams>(() => {
    const salary = parsedParams.get('salary')?.match(/^(\d*)-(\d*)$/)
    const params: JobSearchParams = { page, page_size: PAGE_SIZE, sort: filters.sort }
    const keyword = parsedParams.get('keyword')?.trim()
  ...
  return <div className="min-h-screen bg-slate-50">
    <section className="bg-slate-900 py-12 text-white"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><h1 className="text-3xl font-bold tracking-tight">Tìm việc làm phù hợp</h1><p className="mt-2 max-w-2xl text-sm text-slate-300">Khám phá cơ hội mới và tìm vị trí phù hợp với kinh nghiệm của bạn.</p><div className="mt-7"><JobSearchBar key={`${searchParams.get('keyword') ?? ''}-${searchParams.get('location') ?? ''}`} keyword={searchParams.get('keyword') ?? ''} location={searchParams.get('location') ?? ''} onSearch={(keyword, location) => updateParams({ keyword, location })} /></div></div></section>
    <main className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[260px_minmax(0,1fr)] lg:px-8">
      <JobFilters values={filters} onChange={(next) => updateParams({ employment_type: next.employmentType || null, experience_level: next.experienceLevel || null, salary: next.salary || null, sort: next.sort === 'newest' ? null : next.sort })} onClear={() => setSearchParams({})} />
      <section aria-live="polite"><div className="mb-5"><h2 className="text-xl font-semibold text-slate-900">Danh sách việc làm</h2>{data && <p className="mt-1 text-sm text-slate-500">{data.total} việc làm phù hợp</p>}<QueryFetchFeedback isFetching={isFetching && Boolean(data)} errorMessage={data && errorMessage ? errorMessage : null} onRetry={refetch} /></div>
        {isInitialLoading ? <Skeleton /> : !data && errorMessage ? <div className="rounded-2xl bg-white p-8 text-center shadow-sm ring-1 ring-slate-200"><AlertCircle className="mx-auto text-red-500" size={32} /><h3 className="mt-4 font-semibold text-slate-900">Không thể tải việc làm</h3><p className="mt-2 text-sm text-slate-500">{errorMessage}</p><button type="button" onClick={refetch} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700"><RefreshCw size={15} />Thử lại</button></div> : data?.items.length === 0 ? <div className="rounded-2xl bg-white p-10 text-center shadow-sm ring-1 ring-slate-200"><BriefcaseBusiness className="mx-auto text-blue-600" size={34} /><h3 className="mt-4 font-semibold text-slate-900">Không tìm thấy việc làm phù hợp.</h3><p className="mt-2 text-sm text-slate-500">Thử thay đổi từ khóa hoặc bộ lọc.</p></div> : data ? <><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.items.map((job) => <JobCard key={job.id} job={job} />)}</div>{data.total_pages > 1 && <nav className="mt-8 flex flex-wrap items-center justify-center gap-2" aria-label="Phân trang"><button type="button" disabled={data.page <= 1} onClick={() => updateParams({ page: String(data.page - 1) })} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"><ChevronLeft size={16} />Trước</button>{pages.map((value, index) => <span key={value} className="contents">{index > 0 && value - pages[index - 1] > 1 && <span className="px-1 text-slate-400">…</span>}<button type="button" onClick={() => updateParams({ page: String(value) })} className={`min-w-10 rounded-lg px-3 py-2 text-sm font-semibold ${value === data.page ? 'bg-blue-600 text-white' : 'border border-slate-200 bg-white text-slate-700'}`}>{value}</button></span>)}<button type="button" disabled={data.page >= data.total_pages} onClick={() => updateParams({ page: String(data.page + 1) })} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 disabled:cursor-not-allowed disabled:opacity-40">Sau<ChevronRight size={16} /></button></nav>}</> : null}
      </section>
    </main>
  </div>
}
