...
function getReturnTo(state: unknown) { return state && typeof state === 'object' && 'returnTo' in state && typeof state.returnTo === 'string' ? state.returnTo : '/admin/applications' }
function DetailSkeleton() { return <div className="space-y-6" aria-busy="true"><div className="h-44 animate-pulse rounded-2xl bg-slate-200" /><div className="h-80 animate-pulse rounded-2xl bg-slate-200" /><div className="h-[580px] animate-pulse rounded-2xl bg-slate-200" /></div> }
...
export default function AdminApplicationDetailPage() {
  ...
  useEffect(() => {
    ...
    }).then((response) => { if (active) setApplication(response) }).catch((requestError: unknown) => {
      if (!active) return
