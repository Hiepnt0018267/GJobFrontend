...
export default function AdminCVTemplateEditPage() {
  ...
  const handleSubmit = async (payload: AdminCVTemplateCreateRequest | AdminCVTemplateUpdateRequest, thumbnail?: File) => {
    ...
    try {
      ...
      if (thumbnail) await adminCVTemplateService.uploadAdminCVTemplateThumbnail(id, thumbnail)
  ...
  const handleThumbnailSelected = async (file: File) => {
    ...
    setError(null)
    try {
      setTemplate(await adminCVTemplateService.uploadAdminCVTemplateThumbnail(id, file))
    } catch (requestError) {
      setError(adminCVTemplateErrorMessage(requestError, 'form'))
      throw requestError
    } finally {
